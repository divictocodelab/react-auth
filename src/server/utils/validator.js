...
export default {
    ...
    validateModel(model, rules) {
        ...
        for (let field in rules) {
            ...
            if (ruleTypes.indexOf('enum') !== -1 && isNext) {

                if (value && messages['enum'].options && !this.isValidEnum(value, messages['enum'].options.enum)) {
                    isNext = false;
                    errors.push({ field, message: messages['enum'].message });
                }
            }
        }
        return errors;
    ...
    /**
     * Check field name is exist in array
     * 
     * @param {object} data    
     */
    isValidEnum(value, enumVal) {

        if (Array.isArray(enumVal) && enumVal.indexOf(value) != -1) {
            return true;
        }
        return false;
    },
